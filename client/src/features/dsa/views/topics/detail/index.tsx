...
export default function DsaTopicDetail() {
  ...
  const { data: topic, isLoading: topicLoading } = useQuery<DsaTopic>({
    queryKey: [`/api/dsa-topics/${id}`],
    enabled: !!id,
  });

  const { data: problems = [], isLoading: problemsLoading } = useQuery<DsaProblem[]>({
    queryKey: [`/api/dsa-topics/${id}/problems`],
    enabled: !!id,
  });
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center">
          ...
          <Button onClick={() => setLocation('/dsa-corner/topics')}>
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="bg-gradient-to-r from-blue-500 to-blue-600 text-white py-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <Button 
            ...
            onClick={() => setLocation('/dsa-corner/topics')}
            data-testid="button-back"
          >
            <ArrowLeft className="mr-2 h-4 w-4" />
            Back to Topics
          </Button>
          <h1 className="text-4xl md:text-5xl font-bold mb-4" data-testid="text-topic-name">{topic.name}</h1>
          <p className="text-xl text-blue-50 mb-6" data-testid="text-topic-description">{topic.description}</p>
          <div className="flex gap-4 flex-wrap">
            ...
            <div className="bg-white/10 backdrop-blur-sm rounded-lg px-6 py-3">
              <div className="text-2xl font-bold">{topic.difficulty}</div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <h2 className="text-2xl font-bold mb-6">Problems in this Topic</h2>
        ...
        {problemsLoading ? (
          ...
          <Card>
            <CardContent className="text-center py-12">
              <p className="text-muted-foreground">No problems found for this topic yet.</p>
            </CardContent>
          </Card>
        ) : (
